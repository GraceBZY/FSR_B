...
public class Chapter01 {
	...
	short b = 30; // 声明一个short类型的变量并赋予初始值为10
	int c = 60; // 声明一个int类型的变量并赋予初始值为30
	long d = 40; // 声明一个long类型的变量并赋予初始值为40
	long sum = a + b + c + d;
	System.out.println("20+10+30+40 = " + sum);
	}
	 float e = 12.2f; // 定义float类型并赋予初值
	 double price = 12.2; // 定义double类型的变量并赋予初值
	 float sum2 = e + (float)price;
	 System.out.println(sum2);
	 boolean isable; // 声明 boolean 类型的变量 isable
	 boolean f = false; // 声明 boolean 类型的变量 b，并赋予初值为 false
	 isable = true;
	 char letter = 'D';
	 char numChar = '5';
